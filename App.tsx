...
import { Settings as SettingsType, Message } from './types';
...
import { generateSynthesis, generateQuiz, generateMindmap } from './services/openaiService';
import { Settings, Moon, Sun, Send, PlusCircle, AlertCircle, Search, LogOut } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>('dark'); // Default to dark for "pro" feel
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      setSettings(JSON.parse(savedSettings));
        ...
        // Open settings on first load if keys are missing (only if authenticated)
  ...
  const handleLoginSuccess = () => {
    ...
    // Check if settings need to be opened after login
    const savedSettings = localStorage.getItem('research_agent_settings');
    if (!savedSettings) {
  ...
  const handleConfirmGeneration = async (messageId: string) => {
    ...
    // If generating for a specific message, we usually append the result TO that message or create a new one attached to it.
    // In this "Agent" flow, standard practice is to append a new Agent message showing the result.
    ...
    const initialAgentMsg: Message = {
        ...
        role: 'agent',
        content: '',
        stage: 'synthesizing', // Show loading state
        timestamp: Date.now()
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!settings.openAiKey || !settings.serperKey) {
    ...
    try {
        ...
        // Set agent message to error state
        setMessages(prev => prev.map(msg => 
            msg.id === agentMsgId 
        ...
        <div className="max-w-4xl mx-auto min-h-full flex flex-col justify-start">
            ...
            {messages.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-6 opacity-0 animate-fade-in-up" style={{animationFillMode: 'forwards'}}>
                    ...
                    {(!settings.openAiKey || !settings.serperKey) && (
          ...
          <div className="max-w-4xl mx-auto relative">
              ...
              <div className="text-center mt-3">
                 <p className="text-xs text-gray-400 dark:text-gray-600">
                    Powered by Serper Dev & OpenAI • {settings.model}
                 </p>
              </div>
          </div>
      </footer>
